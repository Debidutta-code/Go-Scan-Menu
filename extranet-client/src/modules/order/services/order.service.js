...
export class OrderService {
    ...
    static async getKOT(token, restaurantId, orderId) {
        try {
            const response = await axiosInstance.get(`/restaurants/${restaurantId}/orders/${orderId}/kot`, { headers: this.getHeaders(token) });
            return response.data;
        }
        catch (error) {
