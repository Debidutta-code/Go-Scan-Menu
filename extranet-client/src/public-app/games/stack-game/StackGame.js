...
function drawBlock(ctx, x, y, w, h, c1, c2, isBase = false, pulse = false) {
    if (w < 1)
        return;
...
export const StackGame = () => {
    ...
    const debrisColors = useRef(new Map());
    ...
    const dropBlock = useCallback(() => {
        ...
        const spawnDebris = (debrisX, debrisW) => {
            if (debrisW < 2)
                return;
            const body = Matter.Bodies.rectangle(debrisX + debrisW / 2, newWorldY + BLOCK_H / 2, debrisW, BLOCK_H - 4, { restitution: 0.35, friction: 0.4, frictionAir: 0.008, label: 'debris' });
        ...
        if (sx < olStart)
            spawnDebris(sx, olStart - sx);
        if (sx + sw > olEnd)
            spawnDebris(olEnd, sx + sw - olEnd);
        // ── Place new block ──
        blocksRef.current = [
            ...blocksRef.current,
    ...
    useEffect(() => {
        ...
        const loop = (t) => {
            ...
            if (phaseRef.current === 'PLAYING') {
                ...
                if (swingXRef.current >= maxX) {
                    swingXRef.current = maxX;
                    swingDirRef.current = -1;
                }
                if (swingXRef.current <= 0) {
                    swingXRef.current = 0;
                    swingDirRef.current = 1;
                }
            }
    ...
    return (_jsxs("div", { className: "sg-wrap", onPointerDown: handleTap, children: [_jsx("canvas", { ref: canvasRef, className: "sg-canvas" }), phase === 'PLAYING' && (_jsxs("div", { className: "sg-hud", children: [_jsx("div", { className: "sg-score-pill", children: score }), _jsx("div", { className: "sg-speed-hint", children: score > 0 && score % 5 === 0 ? '⚡ Speeding up!' : '' })] })), phase === 'IDLE' && (_jsx("div", { className: "sg-overlay", children: _jsxs("div", { className: "sg-card", children: [_jsxs("div", { className: "sg-icon-row", children: [_jsx("span", { className: "sg-block-icon", style: { background: '#ff6b9d' } }), _jsx("span", { className: "sg-block-icon", style: { background: '#ffd32a' } }), _jsx("span", { className: "sg-block-icon", style: { background: '#0be881' } })] }), _jsx("h1", { className: "sg-title", children: "STACK" }), _jsx("p", { className: "sg-desc", children: "Tap to drop \u2014 perfect stacks score more!" }), bestScore > 0 && (_jsxs("div", { className: "sg-best", children: ["Best: ", bestScore] })), _jsx("button", { className: "sg-btn", onClick: e => { e.stopPropagation(); startGame(); }, children: "PLAY" })] }) })), phase === 'OVER' && (_jsx("div", { className: "sg-overlay", children: _jsxs("div", { className: "sg-card", children: [_jsx("div", { className: "sg-over-icon", children: "\uD83D\uDCA5" }), _jsx("h2", { className: "sg-over-title", children: "GAME OVER" }), _jsx("div", { className: "sg-final-num", children: score }), _jsx("div", { className: "sg-final-label", children: "BLOCKS STACKED" }), score >= bestScore && score > 0 && (_jsx("div", { className: "sg-new-best", children: "\u2728 New Best!" })), _jsx("div", { className: "sg-btn-row", children: _jsx("button", { className: "sg-btn", onClick: e => { e.stopPropagation(); startGame(); }, children: "\u21BA RETRY" }) })] }) }))] }));
};
