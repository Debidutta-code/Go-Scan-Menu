...
interface Enemy {
    ...
    type: number; // 0=grunt 1=tank 2=speeder
    flashTimer: number;
}
...
const COLS = 8;
const ROWS = 5;
const SHOOT_CD = 280;       // ms between auto-shots
...
// Draw player ship with canvas paths (modern glowing fighter)
function drawPlayer(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, thruster: number) {
    ctx.save();
    ctx.translate(x + w / 2, y + h / 2);

    // Engine glow
    const eng = ctx.createRadialGradient(0, h * 0.38, 0, 0, h * 0.38, h * 0.32);
    eng.addColorStop(0, `rgba(0,200,255,${0.7 + Math.sin(thruster) * 0.3})`);
    eng.addColorStop(0.4, `rgba(0,100,255,${0.3 + Math.sin(thruster) * 0.15})`);
    eng.addColorStop(1, 'rgba(0,50,200,0)');
    ctx.fillStyle = eng;
    ctx.beginPath();
    ctx.ellipse(0, h * 0.38, h * 0.22, h * 0.32, 0, 0, Math.PI * 2);
    ctx.fill();

    // Thruster flame
    const fl = 0.65 + Math.sin(thruster * 3) * 0.25;
    const flame = ctx.createLinearGradient(0, h * 0.25, 0, h * 0.55 * fl);
    flame.addColorStop(0, 'rgba(255,255,255,0.95)');
    flame.addColorStop(0.25, 'rgba(0,220,255,0.85)');
    flame.addColorStop(0.6, 'rgba(0,80,255,0.5)');
    flame.addColorStop(1, 'rgba(0,20,180,0)');
    ctx.fillStyle = flame;
    ctx.beginPath();
    ctx.moveTo(-w * 0.08, h * 0.25);
    ctx.bezierCurveTo(-w * 0.18, h * 0.4, -w * 0.05, h * 0.55 * fl, 0, h * 0.55 * fl);
    ctx.bezierCurveTo(w * 0.05, h * 0.55 * fl, w * 0.18, h * 0.4, w * 0.08, h * 0.25);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(0, -h * 0.5);
    ctx.bezierCurveTo(w * 0.12, -h * 0.2, w * 0.5, h * 0.1, w * 0.48, h * 0.3);
    ...
    // Cockpit glass
    const glass = ctx.createRadialGradient(-w * 0.05, -h * 0.22, 0, 0, -h * 0.18, w * 0.18);
    glass.addColorStop(0, 'rgba(200,240,255,0.95)');
    glass.addColorStop(0.5, 'rgba(56,189,248,0.6)');
    glass.addColorStop(1, 'rgba(14,165,233,0.2)');
    ctx.fillStyle = glass;
    ctx.beginPath();
    ctx.ellipse(0, -h * 0.18, w * 0.14, h * 0.16, 0, 0, Math.PI * 2);
    ctx.fill();

    // Wing accent lines
    ctx.strokeStyle = 'rgba(147,210,255,0.6)';
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(0, -h * 0.45); ctx.lineTo(0, h * 0.2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(-w * 0.28, h * 0.12); ctx.lineTo(w * 0.28, h * 0.12); ctx.stroke();

    // Gun barrels
    ctx.fillStyle = '#93c5fd';
    ctx.fillRect(-w * 0.42, -h * 0.08, w * 0.06, h * 0.22);
    ctx.fillRect(w * 0.36, -h * 0.08, w * 0.06, h * 0.22);

    ctx.restore();
}

// Draw alien enemy - modern geometric design
function drawEnemy(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, type: number, flash: boolean, t: number) {
    ctx.save();
    ctx.translate(x + w / 2, y + h / 2);

    const colors = [
        { body: '#a855f7', glow: '#d946ef', accent: '#f0abfc' },  // grunt - purple
        { body: '#ef4444', glow: '#f97316', accent: '#fca5a5' },   // tank - red
        { body: '#10b981', glow: '#06d6a0', accent: '#6ee7b7' },   // speeder - green
    ];
    ...
    const pulse = Math.sin(t * 0.003 + type) * 0.06 + 1;
    ctx.scale(pulse, pulse);

    // Core body
    const bodyGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, w * 0.4);
    bodyGrad.addColorStop(0, flash ? '#ffffff' : c.accent);
    bodyGrad.addColorStop(0.5, flash ? '#ffcccc' : c.body);
    bodyGrad.addColorStop(1, flash ? '#ff4444' : c.glow);
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const ry = Math.sin(angle) * h * 0.42;
    ...
    // Wings
    ctx.fillStyle = flash ? 'rgba(255,200,200,0.7)' : `${c.body}bb`;
    // Left wing
    ctx.beginPath();
    ...
    ctx.shadowBlur = 10;
    const eye = ctx.createRadialGradient(0, 0, 0, 0, 0, w * 0.15);
    eye.addColorStop(0, '#ffffff');
    eye.addColorStop(0.4, c.accent);
    eye.addColorStop(1, c.body);
    ctx.fillStyle = eye;
    ctx.beginPath();
    ctx.ellipse(0, 0, w * 0.15, h * 0.15, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    for (let i = -1; i <= 1; i += 2) {
        ctx.beginPath();
        ctx.moveTo(i * w * 0.3, -h * 0.05);
        ctx.lineTo(i * w * 0.52, -h * 0.18);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(i * w * 0.3, h * 0.05);
        ctx.lineTo(i * w * 0.52, h * 0.18);
    ...
    // Type indicator - antenna/spikes
    if (type === 1) { // tank - armored top spikes
        ctx.fillStyle = c.accent;
        for (let i = -1; i <= 1; i++) {
            ctx.beginPath();
            ctx.moveTo(i * w * 0.15, -h * 0.38);
            ctx.lineTo(i * w * 0.15 - w * 0.04, -h * 0.52);
            ctx.lineTo(i * w * 0.15 + w * 0.04, -h * 0.52);
            ctx.closePath();
            ctx.fill();
        }
    }
    if (type === 2) { // speeder - swept wings
...
function drawBullet(ctx: CanvasRenderingContext2D, x: number, y: number, t: number) {
    ctx.save();
...
function drawEnemyBullet(ctx: CanvasRenderingContext2D, x: number, y: number, type: number) {
    ctx.save();
    const colors = ['#e879f9', '#f97316', '#34d399'];
    const c = colors[type % 3];
    ctx.shadowColor = c;
    ctx.shadowBlur = 10;
    ...
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.ellipse(x, y + 5, 3, 9, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
}
...
export const GalaxyShooter: React.FC = () => {
    ...
    // Game objects in refs (mutated each frame)
    ...
    const invincibleRef = useRef(0); // ms of invincibility after hit

    const spawnWave = useCallback((canvas: HTMLCanvasElement, waveNum: number) => {
        const enemies: Enemy[] = [];
        const startX = (canvas.width - COLS * (ENEMY_W + 12)) / 2;
        for (let r = 0; r < ROWS; r++) {
            for (let c = 0; c < COLS; c++) {
                const type = r === 0 ? 1 : r === ROWS - 1 ? 2 : 0; // top=tank, bottom=speeder, rest=grunt
    ...
    useEffect(() => {
        ...
        const loop = (ts: number) => {
            ...
            bg.addColorStop(0.4, '#03001e');
            bg.addColorStop(0.7, '#07023b');
            ...
            explosionsRef.current.forEach(e => {
                ...
                eg.addColorStop(0.6, e.color + '88');
            ...
            if (state === 'PLAYING') {
                ...
                if (shootTimerRef.current >= SHOOT_CD) {
                    shootTimerRef.current = 0;
                    bulletsRef.current.push({ x: px.x + PLAYER_W / 2, y: px.y, active: true });
                    bulletsRef.current.push({ x: px.x + PLAYER_W * 0.15, y: px.y + 10, active: true });
                    bulletsRef.current.push({ x: px.x + PLAYER_W * 0.85, y: px.y + 10, active: true });
                }

                // ── Enemy movement ───────────────────────────────────────────
                const waveSpeed = 28 + waveRef.current * 5;
                const activeEnemies = enemiesRef.current.filter(e => e.active);
                const step = (waveSpeed * dt) / 1000;
                const maxOffset = 60 + waveRef.current * 8;

                enemyOffsetXRef.current += step * enemyDirRef.current;
                ...
                const eshootCd = Math.max(600, ENEMY_SHOOT_CD - waveRef.current * 80);
                if (enemyShootTimerRef.current >= eshootCd && activeEnemies.length > 0) {
                    enemyShootTimerRef.current = 0;
                    // Pick a few random enemies to shoot
                    const shooters = activeEnemies.sort(() => Math.random() - 0.5).slice(0, Math.min(3, Math.ceil(waveRef.current / 2)));
                    shooters.forEach(e => {
                        const ex = e.x + enemyOffsetXRef.current + ENEMY_W / 2;
                        const ey = e.y + enemyOffsetYRef.current + ENEMY_H;
                        const px2 = playerRef.current.x + PLAYER_W / 2;
                        const py2 = playerRef.current.y;
                        const dist = Math.hypot(px2 - ex, py2 - ey);
                        const spd = 2.5 + waveRef.current * 0.3;
                        enemyBulletsRef.current.push({
                            x: ex, y: ey,
                            vx: ((px2 - ex) / dist) * spd,
                            vy: ((py2 - ey) / dist) * spd + 1.5,
                            active: true,
                        });
                    });
                }
                    ...
                    enemiesRef.current.forEach(e => {
                        ...
                        if (b.x > ex && b.x < ex + ENEMY_W && b.y > ey && b.y < ey + ENEMY_H) {
                            ...
                            if (e.hp <= 0) {
                                ...
                                const expColors = ['#a855f7', '#ef4444', '#10b981'];
                                explosionsRef.current.push({ x: ex + ENEMY_W / 2, y: ey + ENEMY_H / 2, r: 5, maxR: 40, life: 1, color: expColors[e.type % 3] });
                                spawnParticles(ex + ENEMY_W / 2, ey + ENEMY_H / 2, expColors[e.type % 3], 16);
                            }
                        }
                    });
                });
                ...
                if (invincibleRef.current <= 0) {
                    ...
                    enemyBulletsRef.current.forEach(b => {
                        if (!b.active) return;
                        if (b.x > px2.x + 6 && b.x < px2.x + PLAYER_W - 6 && b.y > px2.y + 8 && b.y < px2.y + PLAYER_H - 8) {
                            b.active = false;
                ...
                if (activeEnemies.length === 0) {
                    ...
                    setWave(newWave);
                    spawnWave(canvas, newWave);
                    enemyBulletsRef.current = [];
                    // Bonus score per wave
                    scoreRef.current += 100 * (newWave - 1);
                    setScore(scoreRef.current);
                }

                // Enemies reach player
                enemiesRef.current.forEach(e => {
                    if (e.active && e.y + enemyOffsetYRef.current + ENEMY_H > playerRef.current.y) {
                        setGameState('GAMEOVER');
                        gsRef.current = 'GAMEOVER';
                        const hi = Math.max(scoreRef.current, hiScore);
                        setHiScore(hi);
            ...
            // ── Draw bullets ──────────────────────────────────────────────────
            bulletsRef.current.forEach(b => { if (b.active) drawBullet(ctx, b.x, b.y, ts); });
            enemyBulletsRef.current.forEach(b => {
                if (b.active) {
                    // figure out which enemy type shot it -- approximate
                    drawEnemyBullet(ctx, b.x, b.y, 0);
                }
            });
            ...
            enemiesRef.current.forEach(e => {
                ...
                if (e.maxHp > 1) {
                    ...
                    ctx.fillRect(ex, ey - 7, ENEMY_W, 4);
                    const hpGrad = ctx.createLinearGradient(ex, 0, ex + ENEMY_W, 0);
                    hpGrad.addColorStop(0, '#f97316');
                    hpGrad.addColorStop(1, '#ef4444');
                    ctx.fillStyle = hpGrad;
                    ctx.fillRect(ex, ey - 7, ENEMY_W * (e.hp / e.maxHp), 4);
                }
            });

            // ── Draw player ───────────────────────────────────────────────────
            const inv = invincibleRef.current;
            const showPlayer = inv <= 0 || Math.floor(ts / 120) % 2 === 0;
            if (showPlayer) {
                const px = playerRef.current;
                if (inv > 0) ctx.globalAlpha = 0.6;
                drawPlayer(ctx, px.x, px.y, PLAYER_W, PLAYER_H, ts * 0.005);
                ctx.globalAlpha = 1;
                ...
                if (inv > 0) {
                    ...
                    ctx.shadowBlur = 12;
                    ctx.beginPath();
                    ctx.arc(px.x + PLAYER_W / 2, px.y + PLAYER_H / 2, PLAYER_W * 0.72, 0, Math.PI * 2);
                    ctx.stroke();
                    ctx.restore();
                }
            }
    ...
    const onTouchStart = (e: React.TouchEvent) => {
        ...
        if (gsRef.current === 'IDLE' || gsRef.current === 'GAMEOVER' || gsRef.current === 'WIN') startGame();
    };
    ...
    return (
        <div className="gs-container" ref={containerRef}>
            ...
            <canvas
                ...
                onClick={gameState === 'IDLE' || gameState === 'GAMEOVER' || gameState === 'WIN' ? startGame : undefined}
            ...
            {gameState === 'IDLE' && (
                <div className="gs-overlay">
                    <div className="gs-card">
                        ...
                        <p className="gs-desc">Move to aim · Bullets fire automatically</p>
                        <div className="gs-legend">
                            <span>🟣 Grunt — 10pts</span>
                            <span>🔴 Tank — 30pts (2 hits)</span>
                            <span>🟢 Speeder — 20pts</span>
                        </div>
                        {hiScore > 0 && <p className="gs-hi">HI {hiScore}</p>}
                        <button className="gs-btn" onClick={startGame}>TAP TO LAUNCH</button>
                    </div>
                </div>
            )}
            ...
            {gameState === 'GAMEOVER' && (
                <div className="gs-overlay">
                    <div className="gs-card gs-gameover-card">
                        ...
                        <p className="gs-wave-reached">Wave {wave} reached</p>
