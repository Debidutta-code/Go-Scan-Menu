...
function drawPlayer(ctx, x, y, w, h, t) {
    const cx = x + w / 2;
    const cy = y + h / 2;
    ctx.save();
    ctx.translate(cx, cy);
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(-w * 0.27, h * 0.11);
    ctx.lineTo(w * 0.27, h * 0.11);
    ctx.stroke();
    // Gun barrels
...
function drawEnemy(ctx, x, y, w, h, type, flash, t) {
    const COLORS = [
    ...
    for (let i = -1; i <= 1; i += 2) {
        ctx.beginPath();
        ctx.moveTo(i * sw * 0.32, -sh * 0.06);
        ctx.lineTo(i * sw * 0.54, -sh * 0.2);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(i * sw * 0.32, sh * 0.06);
        ctx.lineTo(i * sw * 0.54, sh * 0.2);
...
function drawEnemyBullet(ctx, x, y) {
    ctx.save();
...
export const GalaxyShooter = () => {
    ...
    const spawnWave = useCallback((canvas, waveNum) => {
        const enemies = [];
    ...
    useEffect(() => {
        ...
        const loop = (ts) => {
            ...
            if (state === 'PLAYING') {
                ...
                // Flash timers
                enemiesRef.current.forEach(e => { if (e.flashTimer > 0)
                    e.flashTimer -= dt; });
                // Enemy shoot
                ...
                // Move player bullets
                const bspd = 9 * (dt / 16);
                bulletsRef.current.forEach(b => { if (b.active)
                    b.y -= bspd; });
                ...
                if (invincibleRef.current <= 0) {
                    enemyBulletsRef.current.forEach(b => {
                        ...
                        if (b.x > px2.x + 6 && b.x < px2.x + PLAYER_W - 6 &&
                ...
                const breached = enemiesRef.current.some(e => e.active && e.y + enemyOffsetYRef.current + ENEMY_H > playerRef.current.y);
                if (breached) {
                    setGameState('GAMEOVER');
                    gsRef.current = 'GAMEOVER';
                    const hi = Math.max(scoreRef.current, hiScore);
            ...
            enemyBulletsRef.current.forEach(b => { if (b.active)
            ...
            if (showPlayer) {
                ...
                if (inv > 0) {
                    ...
                    ctx.shadowBlur = 12;
                    ctx.beginPath();
                    ctx.arc(playerRef.current.x + PLAYER_W / 2, playerRef.current.y + PLAYER_H / 2, PLAYER_W * 0.72, 0, Math.PI * 2);
                    ctx.stroke();
                    ctx.restore();
                }
            }
    ...
    return (_jsxs("div", { className: "gs-container", ref: containerRef, children: [gameState === 'PLAYING' && (_jsxs("div", { className: "gs-hud", children: [_jsxs("div", { className: "gs-pill", children: [_jsx("span", { className: "gs-pill-label", children: "SCORE" }), _jsx("span", { className: "gs-pill-value", children: score })] }), _jsxs("div", { className: "gs-pill gs-wave-pill", children: [_jsx("span", { className: "gs-pill-label", children: "WAVE" }), _jsx("span", { className: "gs-pill-value", children: wave })] }), _jsxs("div", { className: "gs-pill", children: [_jsx("span", { className: "gs-pill-label", children: "LIVES" }), _jsx("span", { className: "gs-pill-value gs-lives", children: '♦'.repeat(Math.max(0, lives)) })] })] })), _jsx("canvas", { ref: canvasRef, className: "gs-canvas", onMouseMove: onMouseMove, onTouchMove: onTouchMove, onTouchStart: onTouchStart, onClick: gameState !== 'PLAYING' ? startGame : undefined, style: { touchAction: 'none' } }), gameState === 'IDLE' && (_jsx("div", { className: "gs-overlay", children: _jsxs("div", { className: "gs-card", children: [_jsxs("div", { className: "gs-title-group", children: [_jsx("span", { className: "gs-eyebrow", children: "GALAXY" }), _jsx("h1", { className: "gs-title", children: "SHOOTER" }), _jsx("div", { className: "gs-title-line" })] }), _jsx("p", { className: "gs-desc", children: "Slide to aim \u00B7 Shoots automatically" }), _jsxs("div", { className: "gs-legend", children: [_jsx("span", { children: "\uD83D\uDFE3 Grunt \u2014 10pts" }), _jsx("span", { children: "\uD83D\uDD34 Tank \u2014 30pts \u00B7 needs 2 hits" }), _jsx("span", { children: "\uD83D\uDFE2 Speeder \u2014 20pts" })] }), hiScore > 0 && _jsxs("p", { className: "gs-hi", children: ["BEST \u00A0 ", hiScore] }), _jsx("button", { className: "gs-btn", onClick: startGame, children: "LAUNCH" })] }) })), gameState === 'GAMEOVER' && (_jsx("div", { className: "gs-overlay", children: _jsxs("div", { className: "gs-card gs-gameover-card", children: [_jsx("h2", { className: "gs-over-title", children: "GAME OVER" }), _jsxs("div", { className: "gs-score-row", children: [_jsxs("div", { className: "gs-score-block", children: [_jsx("span", { className: "gs-score-label", children: "SCORE" }), _jsx("span", { className: "gs-score-val", children: score })] }), _jsxs("div", { className: "gs-score-block", children: [_jsx("span", { className: "gs-score-label", children: "BEST" }), _jsx("span", { className: "gs-score-val gs-hi-val", children: hiScore })] })] }), _jsxs("p", { className: "gs-wave-reached", children: ["Reached Wave ", wave] }), _jsx("button", { className: "gs-btn gs-retry-btn", onClick: startGame, children: "RETRY" })] }) }))] }));
};
