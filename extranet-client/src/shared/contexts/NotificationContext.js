...
export const NotificationProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const handleIncomingOrder = (order, eventName) => {
            ...
            const incomingBranchId = typeof order.branchId === 'object'
                ? order.branchId._id?.toString() || order.branchId.toString()
                : order.branchId?.toString();
            // Collect all branches this staff can view
            const allowedBranchIds = (currentStaff.allowedBranchIds || []).map(id => typeof id === 'object' ? id._id?.toString() || id.toString() : id.toString());
            if (currentStaff.branchId) {
                const sbid = typeof currentStaff.branchId === 'object'
                    ? currentStaff.branchId._id?.toString() || currentStaff.branchId.toString()
                    : currentStaff.branchId.toString();
                if (!allowedBranchIds.includes(sbid))
                    allowedBranchIds.push(sbid);
            }
        ...
        const onOrderStaff = (order) => handleIncomingOrder(order, 'orders:send-order-to-staff');
        const onOrderCreated = (order) => handleIncomingOrder(order, 'order:created');
        const onOrderStatus = (order) => handleIncomingOrder(order, 'order:status-update');
    ...
    return (_jsxs(NotificationContext.Provider, { value: { showNotification }, children: [children, notification && (_jsxs("div", { className: "global-notification-banner", style: {
                    position: 'fixed',
                    top: '20px',
                    right: '20px',
                    ...
                    ` }), _jsx("div", { style: {
                        ...
                        }, children: notification.type === 'order' ? _jsx(Bell, { size: 18 }) : _jsx(CheckCircle2, { size: 18 }) }), _jsxs("div", { style: { flex: 1 }, children: [_jsx("div", { style: { fontWeight: 600, fontSize: '14px' }, children: notification.type === 'order' ? 'New Order!' : 'Notification' }), _jsx("div", { style: { fontSize: '13px', opacity: 0.9 }, children: notification.message })] }), _jsx("button", { onClick: () => setNotification(null), style: {
                            background: 'none',
                            border: 'none',
                            color: 'white',
                            cursor: 'pointer',
