...
export class BranchController {
  ...
  createBranch = catchAsync(async (req: Request, res: Response) => {
    const restaurantId =
      ParamsUtil.getString(req.params.restaurantId) || (req as any).user.restaurantId;
  ...
  assignManager = catchAsync(async (req: Request, res: Response) => {
    const restaurantId =
      ParamsUtil.getString(req.params.restaurantId) || (req as any).user.restaurantId;
    const branch = await this.branchService.assignManager(
      ParamsUtil.getString(req.params.id),
      restaurantId,
      req.body
    );
  ...
  deleteBranch = catchAsync(async (req: Request, res: Response) => {
    const restaurantId =
      ParamsUtil.getString(req.params.restaurantId) || (req as any).user.restaurantId;
    const branch = await this.branchService.deleteBranch(
      ParamsUtil.getString(req.params.id),
      restaurantId
    );
    sendResponse(res, 200, {
